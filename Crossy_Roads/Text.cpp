...
vec2 Text::getQuadSize(const string& str, int size) {
	vec2 ret;
	vec2 pos(0);
	for (unsigned int i = 0; i < str.length(); i++) {
		if (i == str.length() - 1) {
			vec2 coord = vec2(pos.x + (float(size) / fontSize) * chars[str[i] - 32].bl, pos.y - (float(size) / fontSize) * chars[str[i] - 32].bt);
			vec2 scale = (float(size) / fontSize) * vec2(chars[str[i] - 32].sx, chars[str[i] - 32].sy);
			ret = coord + scale;
		}
		pos.x += (float(size) / fontSize) * chars[str[i] - 32].ax;
	}
